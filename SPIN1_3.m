...
%     cenTex3({'+'},window,screenRect,colors.black,white,32) % Display fixation cross
    ...
    [y,Fs]=wavread(fileStr); % Read .wav file
    wavplay(y,Fs); % Play .wav file
    tic
...
%     cenTex3({''},window,screenRect,colors.black,white,32) % Display fixation cross
    if debug~=1
        Response=getTextResponses(window,'Target Sentence: ',Xorigin-Xorigin/1.2,Yorigin, colors.black, white);
    else
        Response=corrResp;
    end
    time2=toc;
    ...
    elseif strcmpi(stims{trialVar(i),3},'2T')==1
        score_2t=score_2t+points;
        poss2t=poss2t+possPoints;
    elseif strcmpi(stims{trialVar(i),3},'8T')==1
...
%     if debug~=1;getResp('space');end; % Wait for user to press 'space bar'; % Wait for user to press 'space bar'  
    ...
    data{trialN,2}=stims{i,1}; %Stimulus file name
    data{trialN,3}=stims{i,2}; %Stimulus sentence
    data{trialN,4}=stims{i,3}; %Stimulus file type (noise type)
    data{trialN,5}=stims{i,4}; %keywords used for correct response feedback
...
overall_1t=score_1t/poss1t;
overall_2t=score_2t/poss2t;
overall_8t=score_8t/poss8t;
overall_ssn=score_ssn/possssn;
overall=totalpoints/totPos;
...
instructions = ['You have reached the end of the experiment',...
    ...
    '',...
    ['You got ' overall_1t '% of the 1 talker noise keywords correct!'],...
    ['You got ' overall_2t '% of the 2 talker noise keywords correct!'],...
    ['You got ' overall_8t '% of the 8 talker noise keywords correct!'],...
    ['You got ' overall_ssn '% of the white noise keywords correct!'],...
    ['And you got ' overall '% of the all words correct!'],...
    '',...
    'Thanks!'];
